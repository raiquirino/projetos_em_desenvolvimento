...
if not arquivos_pdf:
    ...
    if 0 <= escolha < len(arquivos_pdf):
       ...
       # Como não conseguia trazer os valore com C e D fiz com que ele identificasse o o C ou o D e colocasse no final do valor      
        ...
        ############################################################################################################'''      
        # Retira numeros e carateres especiais
        df['Linha'] = df['Linha'].apply(lambda x: re.sub(r'[^a-zA-Z\s]', '', str(x)) if pd.notnull(x) else x)          
        df['Linha'] = df['Linha'].apply(lambda x: re.sub(r'\d+', '', str(x)) if pd.notnull(x) else x)
        ############################################################################################################'''
        # substitui todo o historico da linha de acordo com um texto contido na linha.
        substituicoes = {
        "TARIFA PIX": "TARIFA BANCARIA",
        "TARIFA PACOTE" : "TARIFA BANCARIA",
        ...
        substituicoes_finais = {
            ...
            ".TRANSF.CONTAS DIF.TIT.": "TRANSF.CONTAS DIF.TIT.",
            "DEBITO PACOTE SERVICOS": "TARIFA BANCARIA",
            "xxxx": "",
            "xxxx": "",
