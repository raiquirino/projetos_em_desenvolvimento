...
if not arquivos_pdf:
    ...
    if 0 <= escolha < len(arquivos_pdf):
       ...
       # padroes_para_remover = [
        ...
        for idx in df.index:
            data_match = re.search(r'\b\d{1,2} DE [A-ZÇ]{3,9} DE \d{4}\b', df.at[idx, 'Linha'])  # Novo padrão de data
            if data_match:
                data = data_match.group(0)  # Captura a data no formato desejado
                df.at[idx, 'Data'] = data

                # Remove a data do texto original e limpa espaços extras
                df.at[idx, 'Linha'] = re.sub(r'\b\d{1,2} DE [A-ZÇ]{3,9} DE \d{4}\b', '', df.at[idx, 'Linha']).strip()
        ############################################################################################################'''
