...
if not arquivos_pdf:
    ...
    if 0 <= escolha < len(arquivos_pdf):
       ...
       # padroes_para_remover = [
        ...
        ############################################################################################################'''
        # Criar coluna Data vazia Formatos, dd/mm/aa, dd/mm/aam e dd/mm/aaaa
        df['Data'] = None
        ...
        '''############################################################################################################'
        # Extrair datas para nova coluna Formato dd/mm/aaaa
        ...
        ############################################################################################################
        # Extrair datas para nova coluna Formato DD/MM
        ...
        for idx in df.index:
            data_match = re.search(
                 r'\b\d{2}/\d{2}\b', df.at[idx, 'Linha'])  # Expressão para DD/MM
            if data_match:
                data = data_match.group(0)
                df.at[idx, 'Data'] = data
                # Remove a data do texto original e limpa espaços extras
                df.at[idx, 'Linha'] = re.sub(
                    r'\b\d{2}/\d{2}\b', '', df.at[idx, 'Linha']).strip()
