...
if not arquivos_pdf:
    ...
    if 0 <= escolha < len(arquivos_pdf):
       ...
       # Extrair datas para nova coluna
        ...
        ############################################################################################################'''
        # Extrair datas para nova coluna FORMATO MM/AAAA

        df['Data'] = None  # Criar coluna Data vazia

        # Para cada linha, verificar se tem data e processar
        for idx in df.index:
            data_match = re.search(r'\b\d{2}/\d{2}\b', df.at[idx, 'Linha'])
            if data_match:
                data = data_match.group(0)
                df.at[idx, 'Data'] = data
                # Remove a data do texto original e limpa espaços extras
                df.at[idx, 'Linha'] = re.sub(r'\b\d{2}/\d{2}\b', '', df.at[idx, 'Linha']).strip()
